// sync-stripe-products-commonjs.js
...
async function syncAllProductsAndPrices() {
  ...
  // First, sync the product with ID prod_RpeDBw9OteYUhH that's causing the issue
  try {
    console.log('Looking up missing product prod_RpeDBw9OteYUhH...');
    const missingProduct = await stripe.products.retrieve('prod_RpeDBw9OteYUhH');
    
    if (missingProduct) {
      console.log(`Found missing product! Syncing ${missingProduct.id} (${missingProduct.name})...`);
      await supabase.from('stripe_products').insert({
        stripe_product_id: missingProduct.id,
        name: missingProduct.name,
        description: missingProduct.description || '',
        active: missingProduct.active,
        metadata: missingProduct.metadata,
        created_at: new Date().toISOString(),
        updated_at: new Date().toISOString()
      });
      
      console.log('Missing product synced successfully!');
    }
  } catch (error) {
    if (error.code === 'resource_missing') {
      console.log('The product prod_RpeDBw9OteYUhH no longer exists in Stripe.');
    } else {
      console.error('Error syncing missing product:', error);
    }
  }
  
  // Then sync the rest of our known products
